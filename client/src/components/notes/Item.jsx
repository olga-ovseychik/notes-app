...
const Item = ({ noteId, text, title, updatedAt, todos }) => {
    ...
    return (
        <div 
            ...
            id={noteId}>
            <div className={`md:text-xl xs:text-base break-words ${!title ? 'text-secTxtColor dark:text-semiLight' : 'text-hoverColor dark:text-zinc-200'}`}>
                {title ? `${title.length > 30 ? `${title.slice(0, 30)}...` : title}` : 'Untitled'} 
            </div>
            <div className={`md:text-base xs:text-sm break-words ${!text ? 'text-secTxtColor dark:text-semiLight' : 'text-hoverColor dark:text-zinc-200'}`}>
                {!text && todos.length == 0 ? `Just start writing something` : `${text.length > 40 ? `${text.slice(0, 40)}...` : text}`}
            </div>
            {todos.length > 0 ?
                 <div>
                    <div className="inline-block relative py-2">
                        <span 
                            className="bg-mainColor dark:bg-mainColor text-textColor text-sm font-medium me-2 px-1.5 py-0.5 rounded text-center">
