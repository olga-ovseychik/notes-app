...
const EditItem = () => {
    ...
    const handleTagInputKeyDown = (evt) => {
        if (evt.key === 'Enter') {
            evt.preventDefault();
    ...
    return (
        <div className='flex flex-col justify-between sm:max-h-dvh sm:min-h-dvh shadow-lg border rounded-md hover:border-light cursor-default bg-white p-10 overflow-auto scroll-smooth dark:bg-darkModeSecColor dark:border-none dark:shadow-darkMode'>
            <div className="flex flex-col">
                <div className="flex flex-row justify-between items-center">
                    ...
                    <div className="relative self-end">
                        ...
                        <ul ref={settingsList} className="hidden absolute right-0 text-sm min-w-36 border rounded-md p-2 mt-2 bg-white shadow-lg">
                            ...
                            <li className="hover:bg-slate-100 cursor-pointer rounded-md p-1 text-red-500" onClick={() => handleRemoveNote(noteId)}><i className="fa-regular fa-trash-can mr-2"></i>Delete note</li>
                            {note?.todo ?
                ...
                {(activeFilter == 'task' && note?.todos.length == 0) || (note?.task && (note.todos.length > 0 || note?.todos.length == 0)) ? 
                    ...
                    : <textarea 
                        ...
                        onBlur={handleEditNote}
                        className="focus:outline-none focus:bg-light rounded-md p-2 bg-inherit text-textColor text-lg resize-none min-h-96 mt-1 dark:focus:bg-hoverColor dark:text-semiLight"
                ...
                <div className="mt-4">
                    <div className="relative inline-block w-20 mr-2">
                        ...
                        <input  
                            ...
                            type="text" 
                            placeholder={`+ tag`}
                            className={`focus:outline-none rounded-md p-2 text-sm bg-slate-100 dark:bg-hoverColor text-slate-600 placeholder:text-zinc-500 dark:placeholder:text-secTxtColor dark:text-semiLight pl-8 py-1 px-2 mr-2 w-full placeholder:text-sm`}/> 
                    </div>
                    {tagsList?.map((tag, i) => {
                            i++;
                            return (
                                <div className="inline-block relative" key={`tag_${noteId}_${i}`}>
                                    <span 
                                        className="bg-mainColor dark:bg-mainColor text-textColor text-sm font-medium me-2 px-2.5 py-0.5 rounded text-center cursor-pointer"
                                        onClick={addTag(tag)}>
                                        {tag}
                                    </span>
                                    <XCircleIcon 
